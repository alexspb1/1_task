...
def main_view(request):
    random_id_tours = random.sample(list(tours), 6)
    context = {
        'title': title,
        ...
        'tours': tours,
        'random_id_tours': random_id_tours,
    }
...
def departure_view(request, departure: str):
    ...
    for key, value in tours.items():
        for param in value:
            if value.get(param) == departure:
                certain_tours[key] = value
    for key, value in certain_tours.items():
        prices.append(value.get('price'))
        nights.append(value.get('nights'))
    min_price = min(prices)
    max_price = max(prices)
    min_nights = min(nights)
    max_nights = max(nights)
    context = {
        'certain_tours': certain_tours,
...
def tour_view(request, tour_number: int):
    tour_params = tours.get(tour_number)
    tour_param_departure = tour_params.get('departure')
    tour_departure = departures.get(tour_param_departure)
    context = {
